...
def build_tree():
    directory = os.path.join(__file__, 'relativisticpy')
    root = TreeNode(object=directory, internal=True)
    for dirpath, dirnames, filenames in os.walk(directory):
        # Map directory path to TreeNode
        path_to_node = {directory: root}
        
        for dirname in dirnames:
            dir_path = os.path.join(dirpath, dirname)
            node = TreeNode(object=dirname, internal=True, parents=path_to_node[dirpath])
            path_to_node[dir_path] = node
            path_to_node[dirpath].children.append(node)
        
        for filename in filenames:
            if filename.endswith('.py'):
                file_path = os.path.join(dirpath, filename)
                file_node = TreeNode(object=filename, internal=False, parents=path_to_node[dirpath])
                path_to_node[dirpath].children.append(file_node)
