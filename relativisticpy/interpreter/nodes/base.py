...
class AstNode(Node):
    ...
    def get_executor(self, implementer: Implementer) -> Callable:
        if not hasattr(implementer, self.callback):
            raise NotImplementedError(f"The object {type(implementer).__name__} does not have the method '{self.callback}()' required to implement the Node: {type(self).__name__}. ")
        else:
    ...
    def get_state(self, implementer: Implementer) -> ScopedState:
        if not hasattr(implementer, 'state'):
            raise NotImplementedError(f"There exists no {type(implementer).__name__}.state property required to implement the Node: {type(self).__name__}. ")
        else:
...
class AssignmentNode(BinaryNode):
    ...
    def __init__(
        ...
        super().__init__(type=NodeType.ASSIGNMENT, position=position, callback=Implementer.assignment.__name__, args=args)
        self.data_type = 'none'
...
class TensorNode(
    ...
    def execute_node(self, implementer: Implementer) -> None:
        # TODO: Maze of IF / ELIF / ELSE statements => There is a lot of improvements to be made in terms of simplicity / simplification / elegance.
        ...
        # If this components of the tensor were set => We do not return anything. We only cache the object
        elif self.component_ast != None:
    ...
    def tensor_cache_retrieval(self, implementer: Implementer):
        ...
        if is_same_indices != None:
            ...
            if tensor == None:  #  => state does not have an instance.
                ...
                components = tensor[implementer.init_indices(self)]
            ...
            return (
                tensor[implementer.init_indices(self)] if implementer.init_indices(self).anyrunnig or self.sub_components_called else tensor
            )
